...
class AstronomieController extends AbstractController
{
    ...
    public function index(): void
    {
        ...
        $articlesAstronomie = $this->entityManager
            ->getRepository(Astronomie::class)
            ->findBy(['verified' => true]);

        $posts = array_map(static function (Astronomie $article): array {
            return [
                'id' => $article->getId(),
                ...
                'date_astronomie' => null,
            ];
        }, $articlesAstronomie);

        $userPlaylist = [];
        foreach ($articlesAstronomie as $article) {
            foreach (explode(',', (string) $article->getMusicFile()) as $file) {
                $file = trim($file);
                if ($file !== '' && !in_array($file, $userPlaylist, true)) {
        ...
        $this->render('astronomie/index', [
            'nombreVisite' => $nombreVisite,
            'isConnected' => $isConnected,
            'userName' => $userName,
            'userEmail' => $userEmail,
            'menuHtml' => $menuHtml,
            'jsonPlaylist' => json_encode($userPlaylist, JSON_UNESCAPED_SLASHES),
            'articlesAstronomie' => $articlesAstronomie,
            'posts' => $posts,
            'launches' => $this->getUpcomingLaunches(),
            'astroEvents' => $this->getUpcomingAstroEvents()
        ]);
    }
    ...
    public function show(?int $id = null): void
    {
        ...
        $articleEntity = $this->entityManager->getRepository(Astronomie::class)->find($id);
        if (!$articleEntity || !$articleEntity->getVerified()) {
            $this->renderError(404);
        }

        $isConnected = isset($_SESSION['email']) && isset($_SESSION['password']);
        $entityClass = $isConnected ? MenuConnect::class : MenuPrincipal::class;
        $menuItems = $this->entityManager->getRepository($entityClass)
            ->findBy([], ['parent' => 'ASC', 'id' => 'ASC']);
        ...
        $playlist = [];
        foreach (explode(',', (string) $articleEntity->getMusicFile()) as $file) {
            $file = trim($file);
            if ($file !== '') {
        ...
        $article = [
            ...
            'title_contenu' => $articleEntity->getTitleContenu(),
            'contenu' => $articleEntity->getContenu(),
            'filename' => $articleEntity->getFilename(),
            'background_img' => $articleEntity->getBackgroundImg(),
            'gallery_images' => $articleEntity->getGalleryImages(),
            'show_images' => $articleEntity->getShowImages(),
            'hud_feed_id' => $articleEntity->getHudFeedId(),
            'date_astronomie' => $articleEntity->getDateAstronomie(),
            'name' => $articleEntity->getUser()->getName()
        ];
        ...
        $this->render('astronomie/contenu', [
            'nombreVisite' => $nombreVisite,
            'article' => $article,
            'isConnected' => $isConnected,
            'userName' => $isConnected ? $_SESSION['name'] : '',
            'userEmail' => $isConnected ? $_SESSION['email'] : '',
            'menuHtml' => $menuHtml,
            'jsonPlaylist' => json_encode($playlist, JSON_UNESCAPED_SLASHES)
        ]);
    }

    private function getUpcomingLaunches(): array
    {
        ...
        if (is_file($cacheFile) && time() - filemtime($cacheFile) < $cacheTime) {
            ...
            $curl = curl_init('https://ll.thespacedevs.com/2.2.0/launch/upcoming/?limit=5');
    ...
    private function getUpcomingAstroEvents(): array
    {
        $events = [];
        $xml = @simplexml_load_file('https://in-the-sky.org/rss.php?feed=v0');
        $now = time();
        ...
        usort($events, static fn(array $first, array $second): int => $first['timestamp'] <=> $second['timestamp']);
